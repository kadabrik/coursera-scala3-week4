...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  private def updateMatrix(matrix: SectorMatrix, b: Body): SectorMatrix = {
    matrix += b
  }

  private def mergeMatrix(a: SectorMatrix, b: SectorMatrix): SectorMatrix = a.combine(b)

  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))(updateMatrix, mergeMatrix)
  }
